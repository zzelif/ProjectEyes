...
from data_preprocessing import process_dataset, balance_dataset, align_data, AugmentWithAU
...
fr_train, fr_val, fr_test, _ = process_dataset(train_data_dir, test_data_dir, img_size=(224, 224), batch_size=batch_size)
...
x_train_img, x_val_img, x_train_au, x_val_au, y_train, y_val, label_map, au_data, class_weights = align_data(train_data_dir, train_au_output_path, img_size)
fr_x_tr, fr_x_val, _, _, fr_y_tr, fr_y_val, fr_lb_map, _, _ = align_data(train_data_dir, train_au_output_path, img_size=(224, 224))

gen_inputs, custom_y_train = AugmentWithAU(x_train_img, x_train_au, y_train, cust_batch, augment)
...
if os.path.exists(model_path):
    ...
    custom_history = train_custom(
        model=custom_model, train_inputs=gen_inputs, y=custom_y_train, val_inputs=[x_val_img, x_val_au],
        y_val=y_val, epochs=50, save_path=f"models/custom/custom-{timestamp}.h5", steps=stp, #class_weight=class_weights
    )
    ...
    frozen_history = train_frozen(
        frozen_model, fr_x_tr, fr_y_tr, fr_x_val, fr_y_val, batch_size, epochs, save_path=f"models/frozen/frozen-{timestamp}.h5"
    )
    print("Evaluating the model...")
    ...
    fine_history = finetune_mobilenet(
        finetuned_model, frozen_x, frozen_y, fr_x_val, fr_y_val, batch_size, epochs=40, save_path=f"models/tuned/tuned_mb-{timestamp}.h5"
    )
    print("Evaluating the model...")
    eval_model(finetuned_model, fr_x_val, fr_y_val)
    plot_training(fine_history, "finetuned model", output=f"metrics/tuned/tuned_metrics-{m_timestamp}.png")
